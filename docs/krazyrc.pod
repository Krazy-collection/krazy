...
This list is added to any EXCLUDE settings found in F<$HOME/.krazy>.
...
This list is added to any CHECK settings found in F<$HOME/.krazy>.
...
along with any found in F<$HOME/.krazy>.

=item TYPES type1[,type2,...]

Check the specified file type(s) only.

This list is added to any TYPES settings found in F<$HOME/.krazy>.

=item EXCLUDETYPES type1[,type2,...]

Do NOT check the specified file type(s).

This list is added to any EXCLUDETYPES settings found in F<$HOME/.krazy>.
...
This settings is combined with any SKIP setting found in F<$HOME/.krazy>.
...
"gitlab" prints json suitable for gitlab reporting <https://docs.gitlab.com/ci/testing/code_quality/#code-quality-report-format>.
...
"kde" (default), see https://community.kde.org/Policies/Kdelibs_Coding_Style

"qt", see https://wiki.qt.io/Qt_Coding_Style
...
=item CPP_INCLUDE_ORDER boolean

Tell the "includes" checker not to look for problems with the ordering of include files.
The default is True.

The boolean value can be any of "yes", "no", "true", "false", "on", "off" (case-insensitive).

=back

=head1 SEE ALSO

krazy2(1), krazy2all(1)

=head1 LICENSE

The Krazy Software is available under the terms of the GPL-2.0-or-later license.
