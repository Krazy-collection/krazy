...
Check the specifed file type(s) only.
...
Do not process any file that maches the specified perl regular expression.
See the perlreftut(1) man page for a description of how to write
...
IGNORESUBS is an administrator directive used only by the krazy2xml(1) program.
...
EXTRASUBS is an administrator directive used only by the krazy2xml(1) program.

=item IGNOREMODS module1[,module2,...]

Do not run Krazy on the specified list of KDE modules.

IGNOREMODS is an administrator directive used only by the krazy2xml(1) program.
